...
export default function ArchivePage() {
  ...
  useEffect(() => {
    let ignore = false;
    (async () => {
      const res = await fetch('/api/signals?status=done,deleted');
      const data = await res.json();
      if (!ignore) {
        const rows = (data.signals ?? []) as SignalRow[];
        rows.sort((a, b) => (b.completed_at ?? b.deleted_at ?? '').localeCompare(a.completed_at ?? a.deleted_at ?? ''));
        setItems(rows);
        setLoading(false);
      }
    })();
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-lg flex-col px-4 pb-16 pt-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed py-14 text-center" style={{ borderColor: 'var(--border)' }}>
          ...
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Anything you mark done or delete shows up here — nothing is ever silently lost.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-3 rounded-xl border p-3" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
              <div className="min-w-0 flex-1">
                <p
                  ...
                  style={{ color: 'var(--text-muted)', textDecoration: item.status === 'done' ? 'line-through' : 'none' }}
                >
                  {item.text}
                </p>
                <p className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>
                  {item.status === 'done' ? formatCompletedDate(item.completed_at) : formatRemovedDate(item.deleted_at)}
                </p>
              </div>
